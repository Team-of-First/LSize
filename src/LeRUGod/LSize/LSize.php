...
class LSize extends PluginBase implements Listener {
    ...
    public function onjoin(PlayerJoinEvent $event){
        ...
        if (!isset($this->db[$name]["SIZE"])){
            $this->db[$name]["SIZE"] = 1;
            $pl->sendMessage("§l§f기본크기가 1로 설정되었습니다");
            $this->onsave();
            $this->setsize($this->db[$name]["SIZE"],$pl);
        }else{
            $this->setsize($this->db[$name]["SIZE"],$pl);
            $pl->sendMessage("§l§f구입한 크기로 변경되었습니다");
        }
    ...
    public function onCommand(CommandSender $sender, Command $command, string $label, array $args): bool {
        ...
        if ($command->getName() == "크기"){
            if ($sender->isOp()){
                if (isset($args[0])){
                    if ((float)$args[0]>=0.1 and (int)$args[0]<=10){

                        $sender->sendMessage("§l§f크기가 변경되었습니다!");
                        ...
                        $sender->sendMessage("§l§fOP권한으로 변경할수있는 최대한계를 넘어섰습니다!");

                        }
                }else{

                    $sender->sendMessage("§l§f크기를 입력해주세요!");
                    return true;
                ...
                $sender->sendMessage("§l§fOP만 사용가능한 명령어입니다!");
                return true;
                ...
                if ($args[0] == "작아지기"){
                    if ($item->getId() == 399 and $item->getName() == "§e☆ §f크기코인 §e☆§r") {
                        ...
                        if ($this->db[$name]["SIZE"] >= 0.6) {
                            ...
                            $this->onsave();
                            $this->setsize($this->db[$name]["SIZE"],$sender);
                            $sender->sendMessage("§l§f크기가 작아졌습니다!");
                            $sender->getInventory()->removeItem(Item::get(399,0,1)->setCustomName("§e☆ §f크기코인 §e☆§r"));
                            return true;

                        } else {

                            $sender->sendMessage("§l§f더 작아지면 안됩니다!");
                            return true;

                        }
                    }else{

                        $sender->sendMessage("§l§f크기코인을 손에 들고 실행해주세요!");
                        return true;
                    ...
                    if ($item->getId() == 399 and $item->getName() == "§e☆ §f크기코인 §e☆§r") {
                        if ($this->db[$name]["SIZE"] <= 1.9) {
                            ...
                            $sender->sendMessage("§l§f더 커지면 안됩니다!");
                            return true;
                    ...
                    $sender->sendMessage("§l§f/크기변경 [커지기|작아지기]");
                    return true;
            ...
            $sender->sendMessage("§l§f당신의 크기는 ".$this->db[$name]["SIZE"]." 입니다");
            return true;
        }
        ...
        if ($bl == 57){
            if ($item->getId() == 399 and $item->getName() == "§e☆ §f크기코인 §e☆§r"){
                if ($this->db[$name]["SIZE"]>=0.6){
                    ...
                    $pl->sendPopup("크기가 0.1 작아졌습니다!");
                    return true;

                }else{

                    $pl->sendMessage("§l§f더 작아지면 안됩니다!");
                    return true;
            ...
            if ($item->getId() == 399 and $item->getName() == "§e☆ §f크기코인 §e☆§r") {
                if ($this->db[$name]["SIZE"] <= 1.9) {
                    ...
                    $this->setsize($this->db[$name]["SIZE"], $pl);
                    $pl->getInventory()->removeItem(Item::get(399,0,1)->setCustomName("§e☆ §f크기코인 §e☆§r"));
                    $pl->sendPopup("크기가 0.1 커졌습니다!");
                    return true;

                } else {

                    $pl->sendMessage("§l§f더 커지면 안됩니다!");
                    return true;
